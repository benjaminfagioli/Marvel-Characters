      ...
      {chdemo && (
        <div id="indexContainer">
          <div className="px-2">
            ...
            <p style={{ maxWidth: "90ch", textWrap: "pretty" }}>
              This is a page who basically works consuming{" "}
              <span className="text-warning">Official Marvel's API</span> from
              developers. D'you like marvel? Well, if so, you're gonna love
              this, search a character, see their info, like description,
              comics, series and stories they're in,{" "}
              <span className="text-warning">navigating</span> in all of these.
            </p>
          ...
          <div id="atroposContainer">
            <MyAtropos>
              <CardCharacter
                name={chdemo?.name}
                img={`${chdemo?.thumbnail.path}.${chdemo?.thumbnail.extension}`}
                id={chdemo?.id}
              />
            </MyAtropos>
          </div>
          ...
          <h6>
            Note: If you notice that some item (comic, serie, character) is
            missing, it's because i filtered only items who have a own image.
          </h6>
        </div>
      )}
    </div>
